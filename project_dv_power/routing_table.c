...
bool estimate_short_path()
{
	...
	if(list_length(nbr_list))
	{
		...
		{
			nearest_neighbor.cost=nearest_neighbor.cost+3;
		}
		return true;
	}
	else
...
void remove_nbr(void *n)
{
	...
	if(!list_length(nbr_list))
	{
		printf("list length zero making gListEmpty true\r\n");
		gListEmpty = true;
	}
}
...
void share_routing_table(void)
{
	...
	if(nearest_neighbor.cost > 30)
	{
		printf("gateway removed from the network\r\n");
